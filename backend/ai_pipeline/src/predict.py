...
class AmmoniaPredictionSystem:
    ...
    def _fallback_power(self, reference_df, window=7):
        """実績発電量が取得できない場合のフォールバック値を返す"""
        series = reference_df['actual_power'].dropna()
        if series.empty:
            return 0.0
        tail = series.tail(min(window, len(series)))
        return float(tail.mean())

    def _find_reference_row(self, reference_df, target_ts):
        """前年同日データを探索し、見つからない場合は直近データを返す"""
        if reference_df.empty:
            return None

        for year_back in range(1, 4):
            try:
                candidate = target_ts - pd.DateOffset(years=year_back)
            except Exception:
                continue

            mask = (
                (reference_df['date'].dt.month == candidate.month) &
                (reference_df['date'].dt.day == candidate.day)
            )
            if mask.any():
                return reference_df.loc[mask].iloc[-1]

        return reference_df.iloc[-1]

    def _extend_dataset(self, df, reference_df, forecast_days):
        """前日までのデータ欠損を前年同日で補完し、指定日数先まで予測対象期間を拡張"""
        if df.empty:
            return df, 0, 0
        ...
        history_cutoff = self._jst_today() - timedelta(days=1)
        last_timestamp = df['date'].max()
        baseline_time = last_timestamp - last_timestamp.normalize()
        history_target_ts = pd.Timestamp(history_cutoff) + baseline_time
        forecast_target_ts = last_timestamp + pd.Timedelta(days=forecast_days)
        target_ts = max(history_target_ts, forecast_target_ts)

        history_days_added = 0
        forecast_days_added = 0
        new_rows = []
        current_ts = last_timestamp

        while current_ts < target_ts:
            current_ts += pd.Timedelta(days=1)
            reference_row = self._find_reference_row(reference_df, current_ts)
            power_value = None
            if reference_row is not None and not pd.isna(reference_row.get('actual_power', np.nan)):
                power_value = float(reference_row.get('actual_power', np.nan))
            if power_value is None or pd.isna(power_value):
                power_value = self._fallback_power(reference_df)

            new_row = {
                'date': current_ts,
                'actual_power': power_value,
                'actual_ammonia': np.nan,
                'is_refill': 0
            }

            if current_ts.date() <= history_cutoff and reference_row is not None:
                new_row['actual_ammonia'] = reference_row.get('actual_ammonia', np.nan)
                new_row['is_refill'] = int(reference_row.get('is_refill', 0))
                history_days_added += 1
            else:
                forecast_days_added += 1
        ...
        if new_rows:
            ...
            print(
                f"データ補完: 前年同日コピー {history_days_added}日, 予測追加 {forecast_days_added}日 (目標日 {target_ts.date()})"
            )
        else:
            print("データ補完: 追加不要 (最新データが要件を満たしています)")

        return df, history_days_added, forecast_days_added
    ...
    def predict(self, data_path, output_path='data/predictions.csv', forecast_days=30):
        """予測実行（空白期間自動検出）"""
        print("=== アンモニア在庫予測開始 ===")
        ...
        reference_df = detect_refill_events(df.copy(), threshold=50)
        df, hist_added, future_added = self._extend_dataset(df, reference_df, forecast_days)
        df = detect_refill_events(df, threshold=50)
        df = self.create_time_features(df)
        df = self.create_lag_features(df)
        df = self.create_consumption_features(df)
        
        print(f"予測期間: {len(df)}日 (履歴補完 {hist_added}日, 予測延長 {future_added}日, horizon={forecast_days}日)")
