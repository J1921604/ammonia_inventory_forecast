...
```mermaid
gantt
    title アンモニア在庫予測ダッシュボード 実装スケジュール（2025-12-15開始、土日・休日考慮）
    dateFormat YYYY-MM-DD
    axisFormat %m/%d
  
    section Phase 1 セットアップ
    T001-T003 環境確認              :done, p1, 2025-12-15, 1d
  
    section Phase 2 基盤整備
    T004 E2Eテスト環境構築          :done, p2_1, 2025-12-16, 2d
...
- 開始日変更: 最初のタスク（p2_1）の日付を変更
- 期間調整: 各タスクの日数（例: `2d`）を変更
- 休日対応: Mermaidは自動的に土日をスキップ（`after`使用時）
